...
#             [+2, +0, +2],
#             [-1, +2, -1]
#         ]]], dtype=torch.float32).repeat((1, len(drop_channel), 1, 1)) / 4.
#         self.avg = torch.tensor([[[
#             [-1, -1, -1],
#             [-1, +8, -1],
#             [-1, -1, -1]
#         ]]], dtype=torch.float32).repeat((1, len(drop_channel), 1, 1)) / 8.
#         # self.avg3 = torch.ones((3, 3), dtype=torch.float32)
#         # self.avg15 = torch.ones((15, 15), dtype=torch.float32)
#         self.drop_channel = drop_channel
#         self.mask = None
#         self.device = None
...
#         x_pad = F.pad(x[:, c], (2, 2, 2, 2), mode='reflect')
#         sigma = F.conv2d(F.conv2d(x_pad, self.avg) ** 2, self.avg)
#         high_sigma = sigma > torch.quantile(sigma, self.p, dim=(2, 3))[..., None, None]
#         self.mask = high_sigma.double()
#         # self.mask = torch.empty(x.shape[0], 1, *x.shape[2:]).bernoulli_(p=self.p)
#         # self.mask = self.mask.repeat((1, len(c), 1, 1))
#         if self.device:
#             self.mask = self.mask.to(self.device)
...
#         self.device = device
#         self.kb = self.kb.to(self.device)
#         # self.avg3 = self.avg3.to(self.device)
#         # self.avg15 = self.avg15.to(self.device)
#         return self
...
class CNN(nn.Module):
    def __init__(
        ...
        self.e11 = nn.Conv2d(in_channels, 1, bias=False, **conv_kw)  # output: 570x570x64
